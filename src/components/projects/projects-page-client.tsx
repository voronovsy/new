...
export default function ProjectsPageClient({ initialProjects, initialIssues }: ProjectsPageClientProps) {
  const [projects, setProjects] = React.useState<Project[]>(initialProjects);
  const [allIssues, setAllIssues] = React.useState<JiraIssue[]>(initialIssues);
  const [selectedProject, setSelectedProject] = React.useState<Project | null>(initialProjects.length > 0 ? initialProjects[0] : null);
  ...
  if (!selectedProject) {
    return (
        <div className="flex min-h-screen w-full flex-col bg-background">
             <Header projects={projects} selectedProject={{id: '', name: 'Загрузка...', description: '', status: 'Активен'}} />
             <main className="flex-1 p-4 sm:p-6 md:p-8 flex items-center justify-center">
                <div className="text-center">
                    <p>Проекты не найдены.</p>
                    <Button size="sm" className="gap-1 mt-4" onClick={handleCreateProject}>
